#!/usr/bin/env node
// Test runner for api/extract-utils.js — run 2
const fs = require('fs');
...
async function makeImageBuffers() {
  ...
  const bmp = await sharp(svg).toFormat('bmp').toBuffer();
  return { png, jpeg, bmp };
}
...
(async () => {
  console.log('Starting extract-utils tests (run 2)...');
  ...
  const tests = [
    ...
    {
      name: 'doc',
      prepare: async () => ({ buffer: Buffer.from('{\\rtf1\\ansi\\deff0{\\fonttbl{\\f0 Arial;}}\\f0\\fs24 Sample RTF content for testing\\par }'), originalFilename: 'sample.doc', mimetype: 'application/msword' })
    },
    {
      name: 'odt',
